...
- Blog posts are managed using Contentful. Content is imported locally as JSON files in `data/posts` before building.
...
- `data/posts` - Contains the blog post JSON files (imported from Contentful)
- `src/lib/data` - Contains the data fetching functions (e.g., `posts.ts` for reading JSON files)
...
- `npm run import` - Import blog posts from Contentful and save them as JSON files
